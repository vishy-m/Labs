 ...
 /**
 * TestGUI by Kris McCoy & Dagar Ahmed   v2.15
 * Run customized tests of various classes/method and custom build your GUI
 * No longer calls toString on user-created objects to determine text for GUI
 * No longer crashes if student has incorrect case in their class names (gives proper Class not found message

 * This version adds:
 * Added getEnumFromName() method to retreive student-coded enums
 * Removed any called toString of user-defined objects for use in generating GUI labels. (This was causing GUI failure on incorrectly written toStrings.)
...
public class TestGUI 
{
    ...
    public static final double V_NUM = 2.15;
            ...
            while (perfect && (expectedFull.hasNext() || eo.hasNext()) && (actualFull.hasNext() || ao.hasNext())) {  //no mistake found and still data to scan                
                ...
                while (perfect && eo.hasNext() && ao.hasNext()) {  //no mistake found and still data to scan
                    ...
                    if (! eWord.equals(aWord))  {
                        ...
                        if (eWord.equals("%int%")) {
                        ...
                        else if (eWord.equals("%double%")) {
                            hasWildcard = true;
                            try {                                
                                Double.parseDouble(aWord);
                                continue; //if it parsed, this is acceptable.  Continue to next token.
                            } catch (NumberFormatException nfe) { }
                        }
                        else if (eWord.equals("%word%")) {  
                            hasWildcard = true;
                            ...
                            ao = new Scanner("");
                            continue; //skip this token
                        } else if (eWord.length() > 5 && eWord.substring(0, 4).equals("%int") && eWord.charAt(eWord.length()-1) == '%') {
                            //ranged int                            
                            hasWildcard = true;
                            String[] bounds = eWord.substring(4, eWord.length()-1).split("-");
                            int lower = Integer.parseInt(bounds[0]);
                            int upper = Integer.parseInt(bounds[1]);
                            int actual;
                            try {                                
                                actual = Integer.parseInt(aWord);
                                if (actual >= lower && actual <= upper)
                                    continue; //if it parsed and it is within range, this is acceptable.  Continue to next token.
                            } catch (NumberFormatException nfe) { }
                        } else if (eWord.length() > 8 && eWord.substring(0, 7).equals("%double") && eWord.charAt(eWord.length()-1) == '%') {
                            //ranged double                         
                            hasWildcard = true;
                            String[] bounds = eWord.substring(7, eWord.length()-1).split("-");
                            double lower = Double.parseDouble(bounds[0]);
                            double upper = Double.parseDouble(bounds[1]);
                            double actual;
                            try {                                
                                actual = Double.parseDouble(aWord);
                                if (actual >= lower && actual <= upper)
                                    continue; //if it parsed and it is within range, this is acceptable.  Continue to next token.
                            } catch (NumberFormatException nfe) { }
                        }

                        //See if they are floating-point values that are within the allowable error
